...
class MAMPPI_V3(MPPI):
    ...
    def _detect_progressive(self, x, weights, costs, noise):
        """Only detect when progress stalls — not just low variance."""
        ...
        if len(self._state_buf) < 10:
            return

        # Progress check: is cost improving significantly?
        recent_costs = self._cost_buf[-10:]
        early_avg = np.mean(recent_costs[:5])
        late_avg = np.mean(recent_costs[5:])
        is_improving = late_avg < early_avg * 0.95  # >5% improvement needed to suppress

        # Stagnation check (fast: 8-step window)
        w = min(8, len(self._state_buf))
        recent = np.array(self._state_buf[-w:])
        var = np.mean(np.var(recent, axis=0))
        scale = np.mean(np.abs(recent)) + 1e-6
        is_stuck = var < 0.004 * scale**2

        if is_improving or not is_stuck:
            return  # Either improving or not stuck

        # Confirmed stuck + not improving → add feature
        ess_avg = np.mean(self._ess_buf[-10:])
        spread = np.std(recent, axis=0)
        radius = max(np.linalg.norm(spread) * 3.0, 0.3)

        # Escape direction
        grads = np.array(self._grad_buf[-10:])
